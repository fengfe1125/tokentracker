...
struct SessionTimelineView: View {
    ...
    var body: some View {
        VStack(alignment:.leading,spacing:10) {
            HStack {
                Picker("状态",selection:$status) { Text("全部结果").tag("all");Text("成功").tag("success");Text("错误").tag("error");Text("拒绝").tag("denied");Text("未知").tag("unknown") }
                Picker("类型",selection:$kind) { Text("全部类型").tag("all");Text("工具").tag("tool");Text("Skill").tag("skill");Text("子 Agent").tag("agent") }
                Picker("证据",selection:$confidence) { Text("全部证据").tag("all");Text("精确").tag("exact");Text("推导").tag("derived") }
            }.labelsHidden()
            Text("请求观察与执行证据分开展示；不会将会话 Token 或费用分摊到工具。结果未知不代表仍在运行。").font(.caption).foregroundStyle(.secondary)
            ScrollView {
                LazyVStack(alignment:.leading,spacing:10) {
                    ...
                    if rows.contains(where:{$0.startedAt == nil && $0.endedAt == nil}) {
                        Text("时间未知").font(.headline)
                        ForEach(rows.filter{$0.startedAt == nil && $0.endedAt == nil},id:\.srcKey) { event in eventRow(event) }
                    }
                    if loading { ProgressView() }
                    if failed { Text("时间线读取失败").foregroundStyle(.red);Button("重试") { Task { await load(reset:rows.isEmpty) } } }
                    if before != nil { Button("加载更早的 200 条") { Task { await load(reset:false) } }.disabled(loading) }
                    if rows.isEmpty && !loading && !failed { ContentUnavailableView("暂无可用活动证据",systemImage:"point.3.connected.trianglepath.dotted") }
                }
            }
    ...
    private var groups:[GroupedEvents] {
        ...
        for event in timed {
            ...
            let label = !event.turnID.isEmpty ? "Turn \(event.turnID)" : !parent.isEmpty ? "调用组 \(parent)" : ""
    ...
    private func eventRow(_ event:ActivityEvent) -> some View {
        VStack(alignment:.leading,spacing:5) {
            HStack {
                ...
                Spacer()
                Text(event.status == "unknown" || event.endedAt == nil ? "结果未知" : activityStatusLabel(event.status)).font(.caption)
            }
            HStack {
                Text(event.eventLayer == .requestFallback ? "请求观察" : event.eventLayer == .lifecycle ? "生命周期" : "执行证据")
                Text(event.confidence == "exact" ? "精确" : "推导")
                if let start=event.startedAt ?? event.endedAt { Text(Date(timeIntervalSince1970:Double(start)/1000),style:.time) }
                if let end=event.endedAt { Text("结束");Text(Date(timeIntervalSince1970:Double(end)/1000),style:.time) }
            ...
            if !event.parentCallID.isEmpty { Text("父调用：\(event.parentCallID)").font(.caption2).textSelection(.enabled) }
